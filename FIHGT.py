...
parser = argparse.ArgumentParser(description='FIHGT')
...
def main(seed):
    ...
    data = data.to(device)
    #model = RGCN(args).to(device)
    model = FIHGT(args).to(device)
...
#    edge_weight = data.edge_weight[index_select_list]
    ...
    def train(epoch):
        ...
        #X_tsne = TSNE(n_components=2).fit_transform(output.cpu().detach().numpy())
    ...
    def test():
        ...
        return acc_test, loss_test, f1, precision, recall, userfeatures,output
...
if __name__ == "__main__":
    ...
    for i, seed in enumerate(args.random_seed):
        ...
        # savemat('a.mat', max_userfeature.to('cpu').detach().numpy())
