...
parser.add_argument('--relation_select', type=int, default=[1,2], nargs='+', help='selection of relations in the graph (0-6)')
#parser.add_argument('--random_seed', type=int, default=[0,1,2,3,4], nargs='+', help='selection of random seeds')
parser.add_argument('--random_seed', type=int, default=[0], nargs='+', help='selection of random seeds')
parser.add_argument('--model', type=str, default='RGCN', help='GCN, GAT, GraphSage, RGCN, BotRGCN')
...
def main(seed):
    #dataset = MGTAB('./Dataset/MGTAB/MGTAB')
    #dataset = Twibot20('./Dataset/Twibot-20')
    dataset = Cresci15('./Dataset/Cresci-15/Cresci-15')
    data = dataset[0]
    ...
    def train(epoch):
        model.train()
        output,userfeatures  = model(data.x, edge_index, edge_type)
    ...
    def test():
        ...
        return acc_test, loss_test, f1, precision, recall,userfeatures ,out
    ...
    for epoch in range(args.epochs):
        acc_val = train(epoch)
        acc_test, loss_test, f1, precision, recall,userfeatures,output = test()
    ...
    return max_acc, max_precision, max_recall, max_f1,max_userfeature,max_output
...
if __name__ == "__main__":
    ...
    for i, seed in enumerate(args.random_seed):
        ...
        acc, precision, recall, f1,max_userfeature,output = main(seed)
        df_output = pd.DataFrame(output, columns=['label'])
        df_output.to_csv(f'output{i}.csv', index=False)

        tsne = TSNE(n_components=2)
        X_tsne = tsne.fit_transform(max_userfeature.cpu().detach().numpy())
        df_tsne = pd.DataFrame(X_tsne, columns=['Dim1', 'Dim2'])
        df_tsne.to_csv(f'feature{i}.csv', index=False)
