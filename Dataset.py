...
class Twibot20(InMemoryDataset):
    ...
    def process(self):
        # Read data into huge `Data` list.
        labels = torch.load(self.root + "/label.pt")
        des_tensor = torch.load(self.root + "/dense_tensor.pt")
        tweets_tensor1 = torch.load(self.root + "/tweets_tensor_p1.pt")
        tweets_tensor2 = torch.load(self.root + "/tweets_tensor_p2.pt")
        tweets_tensor = torch.cat([tweets_tensor1, tweets_tensor2], 0)
        num_prop = torch.load(self.root + "/num_prop.pt")
        category_prop = torch.load(self.root + "/category_prop.pt")
        edge_index = torch.load(self.root + "/edge_index.pt")
        edge_type = torch.load(self.root + "/edge_type.pt")
